...
#count=1
#cn_start=`cat $lxc_path/$new_container/rootfs/tmp/myip`
#echo "cn_start = $cn_start"
...
container_ip=`cat $lxc_path/$new_container/rootfs/tmp/myip`
...
local_host=`/sbin/ip \r|/bin/grep eth0|/bin/grep src|/usr/bin/awk '{print $9}'`
