...
disablesignup)
	sudo echo 'disable_sign_up true' > $HOME/manage.rb
	sudo mv $HOME/manage.rb /etc/opscode-manage/manage.rb
	sudo chown root.root /etc/opscode-manage/manage.rb && sudo chmod 644 /etc/opscode-manage/manage.rb
	sudo /usr/bin/opscode-manage-ctl reconfigure
	if [ $? -eq 0 ]; then
        	echo '{"code":5000,"success":"true","message":"Chef Server Signup Disabled Successfully"}'
        else
        	echo '{"success":"false","code":5101,"message":"Chef Server Signup Disable Failed"}'
       	fi;;

enablesignup)
	sudo echo 'disable_sign_up false' > $HOME/manage.rb
        sudo mv $HOME/manage.rb /etc/opscode-manage/manage.rb
        sudo chown root.root /etc/opscode-manage/manage.rb && sudo chmod 644 /etc/opscode-manage/manage.rb
        sudo /usr/bin/opscode-manage-ctl reconfigure
        if [ $? -eq 0 ]; then
                echo '{"code":5000,"success":"true","message":"Chef Server Signup Enabled Successfully"}'
        else
                echo '{"success":"false","code":5101,"message":"Chef Server Signup Enabled Failed"}'
        fi;;
