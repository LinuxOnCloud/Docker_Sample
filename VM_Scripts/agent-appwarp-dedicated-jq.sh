...
#					Old_DBFile_Name=`cat $HOME/app/appwarp/AppConfig.json|jq '.HSQLDBFile'`
					#Old_DBFile_Name=`cat $HOME/app/appwarp/AppConfig.json|jq '.HSQLDBFile'|rev|cut -d '/' -f1|rev|tr -d '"'`
					#Old_DBFile_Path=`cat $HOME/app/appwarp/AppConfig.json|jq '.HSQLDBFile'|rev|cut -d '/' -f2-|rev|tr -d '"'|grep -v "HSQLDBFile"`
					Old_DBFile=`cat $HOME/app/appwarp/AppConfig.json|jq '.HSQLDBFile'`
					Old_DBFile_Name=`echo $Old_DBFile|rev|cut -d '/' -f1|rev|tr -d '"'`
					Old_DBFile_Path=`echo $Old_DBFile|rev|cut -d '/' -f2-|rev|tr -d '"'|grep -v "HSQLDBFile"`
						...
						if [ $? -eq 0 ] && [ ! -z "$Old_DBFile" ]; then
							...
							#New_DBFile_Name=`cat $HOME/app/appwarp/AppConfig.json|jq '.HSQLDBFile'|rev|cut -d '/' -f1|rev|tr -d '"'`
		                                       	#New_DBFile_Path=`cat $HOME/app/appwarp/AppConfig.json|jq '.HSQLDBFile'|rev|cut -d '/' -f2-|rev|tr -d '"'|grep -v "HSQLDBFile"`
							New_DBFile=`cat $HOME/app/appwarp/AppConfig.json|jq '.HSQLDBFile'`
                		                        New_DBFile_Name=`echo $Old_DBFile|rev|cut -d '/' -f1|rev|tr -d '"'`
		                                        New_DBFile_Path=`echo $Old_DBFile|rev|cut -d '/' -f2-|rev|tr -d '"'|grep -v "HSQLDBFile"`
...
backup)
		...
		Old_DBFile=`cat $HOME/app/appwarp/AppConfig.json|jq '.HSQLDBFile'`
                Old_DBFile_Name=`echo $Old_DBFile|rev|cut -d '/' -f1|rev|tr -d '"'`
		Old_DBFile_Path=`echo $Old_DBFile|rev|cut -d '/' -f2-|rev|tr -d '"'|grep -v "HSQLDBFile"`
		echo "Old_DBFile = $Old_DBFile"
		if [ -z "$Old_DBFile" ]; then
			echo "parse error: Invalid numeric literal"
		else
...
#                New_DBFile_Path=`grep -v "$New_DBFile_Name" $HOME/new_db_p`
		New_DBFile=`cat $HOME/app/appwarp/AppConfig.json|jq '.HSQLDBFile'`
                New_DBFile_Name=`echo $New_DBFile|rev|cut -d '/' -f1|rev|tr -d '"'`
                New_DBFile_Path=`echo $New_DBFile|rev|cut -d '/' -f2-|rev|tr -d '"'|grep -v "HSQLDBFile"`
                warpstop
                if [ -n "$New_DBFile_Path" ]; then
