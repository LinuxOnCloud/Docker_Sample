...
setup)
                ...
                if [ "$logval" = "tomcat" ]; then
                        ...
                        ln -sf $HOME/log/apache2 $HOME/logging/$2
                        echo "$2" > $HOME/DepID
...
backup)
		rm -rf $HOME/App_Backup
                ...
                cp -arf $HOME/phpapp/php $HOME/App_Backup/php
                echo "$HOME/phpapp/php $HOME/App_Backup/php"
                cd $HOME/App_Backup && tar czf php.tar.gz php
		if [ $? -eq 0 ]; then
			rm -rf $HOME/App_Backup/php
                        echo '{"code":5000,"success":"true","message":"App Files Backup Created","appBackupPath":"'$HOME/App_Backup/php.tar.gz'"}'
                else
                        rm -rf $HOME/App_Backup/
...
#                wget --no-check-certificate --directory-prefix=$HOME/download $2
			...
			if [ $? -eq 0 ]; then
                	        cp -arf $HOME/phpapp/php $HOME/backup/
				if [ $? -eq 0 ]; then
					phpstop
					rm -rf $HOME/phpapp/php	
					mv $HOME/download/php $HOME/phpapp/php
					if [ $? -eq 0 ]; then
						phpstart
