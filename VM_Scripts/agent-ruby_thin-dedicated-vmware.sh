...
rubystart() {
	/home/paasadmin/thin/init/thin start
	sudo /etc/init.d/nginx start
}

# stop nginx function
rubystop() {
	/home/paasadmin/thin/init/thin stop
...
cmd)		
		...
		if [ $? -eq 0 ]; then
			...
			echo '{"success":"false", "code":9501,"message":"Command Execution Failed"}'
                fi;;
...
start)
	...
	if [ -n "$state" ] && [ -n "$state1" ] && [ -n "$state2" ] && [ -n "$state3" ]; then
		...
		if [ $? -eq 0 ]; then
			...
			echo '{"success":"false","code":9502, "message":"Nginx Could Not Be Started"}'
		fi
    	fi;;
...
stop)
	...
	if [ -z "$state" ] && [ -z "$state1" ] && [ -z "$state2" ] && [ -z "$state3" ]; then
		...
		if [ $? -eq 0 ]; then
			...
			echo '{"success":"false","code":9503, "message":"Nginx Could Not Be Stopped"}'
		fi
  	fi;;
...
#############################################
					######### ------------######################
							...
							state=`sudo netstat -npl|grep nginx|grep 8080|grep -v worker|head -1|cut -d'/' -f1|rev|awk '{print $1}'|rev`
								...
								rm -rf $HOME/rubyapp/app*
                                                                ...
                                                                rm -rf $HOME/download
                                                                rm -rf $HOME/fname
								echo '{"success":"false","code":9504, "message":"Nginx Could Not Be Started With New Ruby App, Deployment Failed"}'
								exit 1
							fi
                                        	fi
						...
						rm -rf $HOME/download
						rm -rf $HOME/fname
						echo '{"success":"false","code":9505, "message":"Ruby App Deployment Failed"}'
					fi 
					...
					rm -rf $HOME/download
					echo '{"success":"false","code":9506, "message":"Ruby App Contents For Backup Could Not Be Moved"}'
					exit 1
				fi
			else
				rubystart
				# remove downloaded file
				rm -rf $HOME/download
				echo '{"success":"false","code":9507, "message":"Nginx Could Not Be Stopped"}'
	       	        	exit 1
			fi
		else
			...
			rm -rf $HOME/download
			rm -rf $HOME/.aws/config $HOME/url
			echo '{"success":"false","code":9508, "message":"Ruby App Download Failed"}'
			exit 1
	fi;;
...
createkey)
                ...
                if [ ! -z /home/paasuser/.ssh/authorized_keys ]; then
                        ...
                        echo '{"success":"false","code":9509,"message":"PaaSUser SSH key Could Not Be Created"}'
                fi;;
...
#			sudo /sbin/katr sheppaasuseratr
			if [ -L $HOME/logging/$2 ]; then
				...
				echo '{"success":"false","code":9510,"message":"LOG Link Could Not Be Create, Setup is Failed"}'
                	fi
		else
			echo '{"success":"false","code":9511,"message":"LOG Value Could Not Be Match in Apache Server, Setup is Failed"}'
		fi;;
...
addcronjob)
	        ...
	        if [ $? -eq 0 ]; then
        	        ...
        	        echo '{"success":"false","code":9512,"message":"CronJob Could Not Be Added"}'
	        fi;;
...
deletecronjob)
	        ...
	        if [ $? -eq 0 ]; then
        	        ...
        	        echo '{"success":"false","code":9513,"message":"CronJob Could Not Be Deleted"}'
	        fi;;
...
*)
                ...
                echo '{"success":"false", "code":9514,"message":"Invalid Command"}'
                ;;
