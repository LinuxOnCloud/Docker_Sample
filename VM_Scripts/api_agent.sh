				...
				if [ $? -eq 0 ]; then
					...
					if [ $? -eq 0 ]; then
						...
						if [ $? -eq 0 ]; then
							...
							rm -rf $HOME/download	
							echo '{"code":5000,"success":"true", "message":"API App Deployed Successfully"}'
										

#############################################
					######### ------------######################
						else
							echo '{"success":"false", "code":6304,"message":"API App Deployed But Tomcat Could Not Be Started"}' 
							exit 1							
                                        	fi
...
#						sudo rm -rf $HOME/download
						rm -rf $HOME/backup/ROOT*
						echo '{"success":"false","code":6305,"message":"API App Deployment Failed"}'
						exit 1
					fi 
					...
					# remove downloaded file
                                	rm -rf $HOME/download
					echo '{"success":"false", "code":6306,"message":"API App Contents For Backup Could Not Be Moved"}'						
					exit 1
				fi
			else
				tomcatstart
			...
			rm -rf $HOME/download
			rm -rf $HOME/.aws/config $HOME/url
			echo '{"success":"false", "code":6308,"message":"API App Download Failed"}'
			exit 1
		fi;;
...
create_war)
	...
	sudo /etc/init.d/tomcat stop
        cd $HOME/app/App42_API_SERVER/ && git pull
        if [ $? = 0 ]; then
                #/bin/bash $HOME/config_constructor.sh $2 $3 $4 $5 $6 $7 $8
                #/bin/bash $HOME/datasource_constructor.sh $9 ${10} ${11}
                /$HOME/config_constructor $2 $3 $4 $5 $6 $7 $8 $9
                /$HOME/datasource_constructor ${10} ${11} ${12}
                if [ $? = 0 ]; then
                        cd $HOME/app/App42_API_SERVER/ && grails war ROOT.war
                        if [ $? = 0 ]; then
                                mkdir -p $HOME/Warfile
				mv $HOME/app/App42_API_SERVER/ROOT.war $HOME/Warfile/ROOT.war
				rm -rf $HOME/.grails/
				rm -rf $HOME/.ivy2/
				tomcatstart
                                echo '{"code":5000,"success":"true","message":"API WAR File Created Successfully","path":"'$HOME/Warfile/ROOT.war'"}'
                        else
                                rm -rf $HOME/app/App42_API_SERVER/ROOT.war
                                rm -rf $HOME/Warfile/ROOT.war
                                echo '{"success":"false","code":6309,"message":"API WAR File Creation Failed"}'
                        fi

                else
                        rm -rf $HOME/app/App42_API_SERVER/ROOT.war
                        rm -rf $HOME/Warfile/ROOT.war
			echo '{"success":"false","code":6310,"message":"API Service Parameters Could Not Be Changed"}'
                fi
        else
                rm -rf $HOME/app/App42_API_SERVER/ROOT.war
                rm -rf $HOME/Warfile/ROOT.war
                echo '{"success":"false","code":6311,"message":"API Git Pull Could Not Be Successfull"}'
        fi;;
...
delete_war)
	...
	rm -rf $HOME/Warfile
		echo '{"code":5000,"success":"true","message":"API WAR File Deleted Successfully"}'
	else
		echo '{"success":"false","code":6312,"message":"API WAR File Could Not Be Deleted"}'
	fi;;
