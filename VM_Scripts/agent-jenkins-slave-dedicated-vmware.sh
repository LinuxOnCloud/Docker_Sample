...
start)
		...
		slave=`cat $HOME/slavecron|cut -d "/" -f18`
                pid=`ps ax |grep $slave|grep slave.jar|awk '{print $1}'`
		echo "state = $state"
		if [ -n "$state" ] && [ -n "$pid" ]; then  		
                        cat $HOME/cronjob $HOME/slavecron|crontab
			echo '{"code":5000,"success":"true", "message":"Tomcat Already Started"}'
          	else
			/bin/bash $HOME/startslave.sh
			tomcatstart 
			if [ $? -eq 0 ]; then
				cat $HOME/cronjob $HOME/slavecron|crontab
...
artifactory)
        artif_ip=`cat $HOME/.jenkins/org.jfrog.hudson.ArtifactoryBuilder.xml|grep url|cut -d"/" -f3|cut -d "<" -f1`
...
echo '<slave>
  ...
  <remoteFS>/home/paasadmin/.jekins/</remoteFS>
	...
	if [ $? -eq 0 ]; then
		...
		if [ $? -eq 0 ]; then
			...
			sleep 10
        		...
        		tomcatstart
			sleep 30
			curl http://App42:app42jenkins@localhost:8080|grep "create new jobs">$HOME/jenkins-stat
			while [ "$(du -m $HOME/jenkins-stat|cut -f1)" -lt 1 ];
			do
				echo "$HOME/jenkins-stat is Empty"
				sleep 10
				curl http://App42:app42jenkins@localhost:8080|grep "create new jobs">1234
			done
...
runslave)
	java -jar $HOME/tomcat/webapps/ROOT/WEB-INF/slave.jar -jnlpUrl http://$2/computer/$3/slave-agent.jnlp -secret $4 > $HOME/slave.log 2>&1 &
#> /dev/null 2>&1 
	sleep 5
#	nohup /home/paasadmin/java/bin/java -jar /home/paasadmin/tomcat/webapps/ROOT/WEB-INF/slave.jar -jnlpUrl http://$1/computer/$2/slave-agent.jnlp -secret $3 &
	stat=`cat $HOME/slave.log|tail -1|awk '{print $2}'`
	echo "stat = $stat"
	if [ "Connected" == $stat ]; then
		echo "*/1 * * * * $HOME/java/bin/java -jar $HOME/tomcat/webapps/ROOT/WEB-INF/slave.jar -jnlpUrl http://$2/computer/$3/slave-agent.jnlp -secret $4 > $HOME/slave.log 2>&1 &" >$HOME/slavecron
		echo "$HOME/java/bin/java -jar $HOME/tomcat/webapps/ROOT/WEB-INF/slave.jar -jnlpUrl http://$2/computer/$3/slave-agent.jnlp -secret $4 > $HOME/slave.log 2>&1 &" >$HOME/startslave.sh
		chmod +x $HOME/startslave.sh
		tomcatstart		
               	cat $HOME/cronjob $HOME/slavecron|crontab
...
*)
		echo '{"success":"false", "code":9214,"message":"Invalid Command - Jenkins Slave Server"}'
                ;;
esac
