...
if [ -z $process ]; then
        STATUS=$(cat $LOG-$i)
                if [ $STATUS != "$i-PROCESS-DOWN!" ]; then
                        echo "`date`: Process $1 is Down, Host $ip Service $1 is Down!" | mail -s "Host $ip Service $1 Is DOWN!" $EMAIL
        ...
        STATUS=$(cat $LOG-$i)
                if [ $STATUS != "$i-PROCESS-UP!" ]; then
                        echo -e "`date`: Process $1 is Up, Host $ip Service $1 is Up!" | mail -s "Host $ip Service $1 is UP!" $EMAIL && mv $Outfile.$i $Outfile.$i-`date +%d-%m-%Y-%H:%M:%S`
                fi
        echo "$i-PROCESS-UP!" > $LOG-$i
fi
done
