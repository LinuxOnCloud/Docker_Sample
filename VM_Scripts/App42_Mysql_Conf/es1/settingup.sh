...
if [ $? -eq 0 ]; then
	...
	if [ $? -eq 0 ]; then
		sleep 10
		sudo cp -arf /home/azureuser/Installationpkg/es1/app42RDS /.
		if [ $? -eq 0 ]; then
			...
			if [ $? -eq 0 ]; then
				echo "ES1 Configured Successfully"
			else
				echo "ES1 SSH Key Not Installed"
				exit 1
			fi
		else
			echo "ES1 Config Configuration Failed"
			exit 1
		fi
	else
		echo "ES1 Service Could Not Be Installed"
		exit 1
	fi
else
	echo "ES1 Repo Setup Failed"
	exit 1
fi

else
        echo "ES1 CentOS Repo Setup Failed"
        exit 1
fi
