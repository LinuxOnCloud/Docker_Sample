...
if [ $? -eq 0 ]; then
	...
	if [ $? -eq 0 ]; then
		sleep 10
		sudo cp -arf /home/azureuser/Installationpkg/es3/app42RDS /.
		if [ $? -eq 0 ]; then
			...
			if [ $? -eq 0 ]; then
				echo "ES3 Configured Successfully"
			else
				echo "ES3 SSH Key Not Installed"
				exit 1
			fi
		else
			echo "ES3 Config Configuration Failed"
			exit 1
		fi
	else
		echo "ES3 Service Could Not Be Installed"
		exit 1
	fi
else
	echo "ES3 Repo Setup Failed"
	exit 1
fi

else
        echo "ES3 CentOS Repo Setup Failed"
        exit 1
fi
