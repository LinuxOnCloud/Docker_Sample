...
if [ $? -eq 0 ]; then
	sudo sed -i 's/enabled=0/enabled=1/g' /etc/yum.repos.d/mysql-community.repo && sudo yum install -y mysql-community-server-$3 mysql-community-client-$3 mysql-community-common-$3 mysql-community-libs-compat-$3 mysql-community-libs-$3 && sudo cp -arf /home/azureuser/Installationpkg/mysqlha2/app42RDS /.

	sudo chown -R root.root /app42RDS
	if [ $? -eq 0 ]; then
		/home/azureuser/Installationpkg/comman/slave_config poiuytrewq $2
                sudo cp -arf /home/azureuser/Installationpkg/comman/slave.cnf /etc/my.cnf
		...
		if [ $? -eq 0 ]; then
			...
			if [ $? -eq 0 ]; then
				echo "MysqlHA2 Configured Successfully"
			else
