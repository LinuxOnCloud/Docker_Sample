...
#sudo apt-get install -y --force-yes postgresql-9.6 postgresql-client-9.6 postgresql-9.6-repmgr
	...
	if [ $? -eq 0 ]; then
		sleep 10
		sudo cp -arf /home/azureuser/Installationpkg/redisha2/app42RDS /.
		...
		#	sudo cp -arf /home/azureuser/Installationpkg/postgresqlha2/app42RDS /.
			...
			if [ $? -eq 0 ]; then
				echo "RedisHA2 Configured Successfully"
			else
				echo "RedisHA2 SSH Key Not Installed"
				exit 1
			fi
		else
			echo "RedisHA2 Config Configuration Failed"
			exit 1
		fi
	else
		echo "RedisHA2 Service Could Not Be Installed"
		exit 1
	fi
else
	echo "RedisHA2 Repo Setup Failed"
	exit 1
fi

else
        echo "RedisHA2 CentOS Repo Setup Failed"
        exit 1
fi
