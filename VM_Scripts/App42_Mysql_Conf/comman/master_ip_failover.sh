...
#\`iptables -t nat -I PREROUTING -s 0.0.0.0/0 -p tcp -j DNAT --dport 3306 --to-destination 10.51.1.25:3306\`;
#\`ssh \$ssh_user\@\$orig_master_host \\\" \$ssh_stop_vip \\\"\`;
#\`ssh -i /root/.ssh/id_rsa root@10.51.1.7 iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE\`;
#\`ssh -i /root/.ssh/id_rsa root@10.51.1.7 iptables -t nat -I PREROUTING -s 0.0.0.0/0 -p tcp -j DNAT --dport 3306 --to-destination 10.51.1.25:3306\`;
\`/app42RDS/sbin/master_iptables poiuytrewq &\`;
\`/app42RDS/sbin/mail qwertyuiop &\`;
}
...
#\`iptables -t nat -F\`;
#\`iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE\`;
##\`iptables -t nat -A PREROUTING -i eth0 -p tcp --dport 3306 -j DNAT --to 10.51.1.7:3306\`;
#\`iptables -t nat -I PREROUTING -s 0.0.0.0/0 -p tcp -j DNAT --dport 3306 --to-destination 10.51.1.25:3306\`;
#\`ssh \$ssh_user\@\$orig_master_host \\\" \$ssh_stop_vip \\\"\`;
#\`/bin/bash /etc/mha/mail.sh\`
}

sub usage {
print
\"Usage: master_ip_failover --command=start|stop|stopssh|status --orig_master_host=host --orig_master_ip=ip --orig_master_port=port --new_master_host=host --new_master_ip=ip --new_master_port=port\n\";
}" > /home/azureuser/Installationpkg/comman/mha/master_ip_failover

chmod 755 /home/azureuser/Installationpkg/comman/mha/master_ip_failover
