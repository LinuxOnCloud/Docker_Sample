...
if [ $? -eq 0 ]; then
	...
	if [ $? -eq 0 ]; then
		...
		if [ $? -eq 0 ]; then
			...
			if [ $? -eq 0 ]; then
				echo "ES4 Configured Successfully"
			else
				echo "ES4 SSH Key Not Installed"
				exit 1
			fi
		else
			echo "ES4 Config Configuration Failed"
			exit 1
		fi
	else
		echo "ES4 Service Could Not Be Installed"
		exit 1
	fi
else
	echo "ES4 Repo Setup Failed"
	exit 1
fi

else
        echo "ES4 CentOS Repo Setup Failed"
        exit 1
fi
