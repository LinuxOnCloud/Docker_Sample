...
if [ $? -eq 0 ]; then
	...
	if [ $? -eq 0 ]; then
		...
		if [ $? -eq 0 ]; then
			sudo cp -arf /home/azureuser/Installationpkg/mongoproxyha2/app42RDS /.
			...
			if [ $? -eq 0 ]; then
				echo "MongoDBProxyHA2 Configured Successfully"
			else
				echo "MongoDBProxyHA2 SSH Key Not Installed"
				exit 1
			fi
		else
			echo "MongoDBProxyHA2 Config Configuration Failed"
			exit 1
		fi
	else
		echo "MongoDBProxyHA2 Service Could Not Be Installed"
		exit 1
	fi
else
	echo "MongoDBProxyHA2 Repo Setup Failed"
	exit 1
fi

else
        echo "MongoDBProxyHA2 CentOS Repo Setup Failed"
        exit 1
fi
