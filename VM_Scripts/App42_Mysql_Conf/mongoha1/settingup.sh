...
if [ $? -eq 0 ]; then
	...
	if [ $? -eq 0 ]; then
		...
		if [ $? -eq 0 ]; then
			sudo cp -arf /home/azureuser/Installationpkg/mongoha1/app42RDS /.
			...
			if [ $? -eq 0 ]; then
				echo "MongoDBHA1 Configured Successfully"
			else
				echo "MongoDBHA1 SSH Key Not Installed"
				exit 1
			fi
		else
			echo "MongoDBHA1 Config Configuration Failed"
			exit 1
		fi
	else
		echo "MongoDBHA1 Service Could Not Be Installed"
		exit 1
	fi
else
	echo "MongoDBHA1 Repo Setup Failed"
	exit 1
fi

else
        echo "MongoDBHA1 CentOS Repo Setup Failed"
        exit 1
fi
