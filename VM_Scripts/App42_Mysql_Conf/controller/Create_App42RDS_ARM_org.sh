...
if [ $? -eq 0 ]; then
		...
		for ip in $allow_ip
		do
                azure network nsg rule create -p tcp -r inbound -y $counter -u 3306 -c allow   -g $resourcegroup -a $setupName-proxyhansg -n $setupName-ProxynsgRuleMySql-$rule_no -f $ip
		if [ $? -eq 0 ]; then
			echo "$setupName-ProxynsgRuleMySql-$rule_no $counter $ip," >> /tmp/$setupName-proxyhansg
			counter=$((counter+10))
			rule_no=$((rule_no+1))
		else
...
echo "/app42RDS/sbin/Installation_Arm $proxyha1 $proxyha2 $mysqlha1 $mysqlha2 $mysql_conn $setupName $mysql_version"
/app42RDS/sbin/Installation_Arm $proxyha1 $proxyha2 $mysqlha1 $mysqlha2 $mysql_conn $setupName $mysql_version
if [ $? -eq 0 ]; then
...
#/bin/echo '{"code":5000,"success":"true", "message":"App42RDS Setup Completed Successfully", "subscription_id":"'$subscription_id'", "resource_group":"'$resourcegroup'", "region":"'$region'", "storage_account":"'$storage_name'", "storage_account_key":"'$storage_key'", "setup_ip":"'$proxylb'", "nodes":[{"pub_ip":"'$proxyha1'", "private_ip":"10.20.1.5", "type":"proxy1", "name":"'$setupName'-proxyha1"}, {"pub_ip":"'$proxyha2'", "private_ip":"10.20.1.6", "type":"proxy2", "name":"'$setupName'-proxyha2"}, {"pub_ip":"'$mysqlha1'", "private_ip":"10.20.1.7", "type":"master", "name":"'$setupName'-mysqlha1", "data_disk":"'$setupName'-mysqlha1disk1"}, {"pub_ip":"'$mysqlha2'", "private_ip":"10.20.1.8", "type":"slave", "name":"'$setupName'-mysqlha2", "data_disk":"'$setupName'-mysqlha2disk1"}], "setupConfig":{"mysql_user_name":"'$mysql_user_name'", "mysql_password":"'$user_password'", "mysql_database":"'$mysql_databse_name'"}}'

/bin/echo '{"code":5000,"success":"true", "message":"App42RDS Setup Completed Successfully", "subscription_id":"'$subscription_id'", "resource_group":"'$resourcegroup'", "region":"'$region'", "storage_account":"'$storage_name'", "storage_account_key":"'$storage_key'", "setup_ip":"'$proxylb'", "network_security_group":"'$setupName'-proxyhansg", "network_security_group_rule":"'$nsg_rule'",  "nodes":[{"pub_ip":"'$proxyha1'", "private_ip":"10.20.1.5", "type":"proxy1", "name":"'$setupName'-proxyha1"}, {"pub_ip":"'$proxyha2'", "private_ip":"10.20.1.6", "type":"proxy2", "name":"'$setupName'-proxyha2"}, {"pub_ip":"'$mysqlha1'", "private_ip":"10.20.1.7", "type":"master", "name":"'$setupName'-mysqlha1", "data_disk":"'$setupName'-mysqlha1disk1;'$storagetype';'$disksize'"}, {"pub_ip":"'$mysqlha2'", "private_ip":"10.20.1.8", "type":"slave", "name":"'$setupName'-mysqlha2", "data_disk":"'$setupName'-mysqlha2disk1;'$storagetype';'$disksize'"}], "setupConfig":{"user_name":"'$mysql_user_name'", "password":"'$user_password'", "database":"'$mysql_databse_name'"}}'
