...
echo -e "\nStart VM $vm_name \n"
azure vm start -g $RG_Name -n $vm_name
if [ $? -eq 0 ]; then
	echo '{"code":5000,"success":"true", "message":"'$vm_name' VM Started Successfully", "subscription_id":"'$subscription_id'", "resource_group":"'$RG_Name'", "vm_name":"'$vm_name'"}'
else
	echo '{"success":"false","code":3001,"message":"'$vm_name' VM Could Not Be Started"}'
        exit 1
fi
