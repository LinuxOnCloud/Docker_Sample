...
createdb)
		echo 'db.getSiblingDB("admin").addUser( {user: "'$3'", pwd: "'$4'", roles: [ "userAdminAnyDatabase",  "clusterAdmin", "readWriteAnyDatabase", "dbAdminAnyDatabase" ] } )'|mongo
		echo "db.auth('$3', '$4');"|mongo admin -u $3 -p $4 >$HOME/123
...
resetpassword)
		#sudo sed -ie 's/auth/#auth/g' /etc/mongodb.conf
                #mongostop
                #mongostart
                #sleep 1
		echo "db.changeUserPassword('$3', '$5');" |mongo admin -u $3 -p $4
                echo "db.auth('$3', '$5');"|mongo admin -u $3 -p $5 >$HOME/123
                c=`cat $HOME/123 |tail -2|head -1`
		echo "Auth=$c"
                if [ "$c" -eq "1" ]; then
                        #sudo sed -ie 's/#auth/auth/g' /etc/mongodb.conf
...
backup)
		mongodump -host localhost --port 27017  --username $2 --password $3 --out $HOME/dump/`hostname`-`date +%d%b%Y`/
                if [ $? -eq 0 ]; then
			cd $HOME/dump/ && zip -r `hostname`-`date +%d%b%Y`.zip `hostname`-`date +%d%b%Y` && rm -rf `hostname`-`date +%d%b%Y`
                        echo '{"code":5000,"success":"true","message":"MongoDB Database Backup Created","path":"'$HOME/dump/`hostname`-`date +%d%b%Y`.zip'"}'
                else
                        echo '{"success":"false","code":5206,"message":"MongoDB Database Backup Could Not Be Created"}'
			rm -rf $HOME/dump/`hostname`-`date +%d%b%Y`
                fi;;
...
#                wget --no-check-certificate --directory-prefix=$HOME/download $5
			...
			mongodump -host localhost --port 27017  --username $2 --password $3 --out $HOME/dump/`hostname`/
			if [ $? -eq 0 ]; then
...
#				mongorestore -u $2 -p $3  $out
				mongorestore --host localhost --port 27017 --username $2 --password $3 $HOME/download/$out
				...
				if [ $? -eq 0 ]; then
					...
					rm -rf $HOME/download
					rm -rf $HOME/backup/`hostname`

#############################################
                                        ######### ------------######################
				else
                                	echo '{"success":"false","code":5207,"message":"MongoDB Database Restore Failed"}'
					mongorestore --host localhost --port 27017  --username $2 --password $3 $HOME/backup/`hostname`
					rm -rf $HOME/download
                                        rm -rf $HOME/backup/`hostname`
					exit 1
                          	fi
